...
AIOS Forge CLI supports localization through message dictionaries.
...
- Agent templates read `conversation_language` from `.aios-forge/context/project.context.md` and should use it for interaction/output language.
- Localized agent packs live in `.aios-forge/locales/<locale>/agents/` and can be applied to active prompts with `aios-forge locale:apply`.
...
## Add a new locale
1. Run `aios-forge i18n:add <locale>`.
