...
async function runSquadPipeline({ args = [], options = {}, logger = console } = {}) {
  ...
  if (!hasDb) {
    logger.error('Runtime store not initialized. Run: aios-lite runtime:init .');
  ...
  try {
    if (subcommand === 'list') {
      ...
      if (pipelines.length === 0) {
        logger.log('No pipelines found. Create one with: aios-lite squad:pipeline show --sub=create');
    ...
    if (subcommand === 'show') {
      if (!slugArg) {
        logger.error('Usage: aios-lite squad:pipeline [path] --sub=show --pipeline=<slug>');
    ...
    if (subcommand === 'status') {
      if (!slugArg) {
        logger.error('Usage: aios-lite squad:pipeline [path] --sub=status --pipeline=<slug>');
        return { ok: false, error: 'missing_slug' };
      }
      const dag = getPipelineDAG(db, slugArg);
      if (!dag) {
