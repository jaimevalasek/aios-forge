...
## Entrada
- Blueprint em `.aios-forge/squads/.designs/<slug>.blueprint.json`
...
Leia `.aios-forge/squads/.designs/<slug>.blueprint.json` e valide que os campos obrigatórios existem (slug, name, problem, goal, mode, executors).

### Passo 2 — Criar estrutura de diretórios
```
.aios-forge/squads/<slug>/
├── agents/
...
Para cada executor no blueprint, crie `.aios-forge/squads/<slug>/agents/<executor-slug>.md` seguindo o template atual do squad.md (seção "Step 2 — Generate each specialist agent"):
...
Crie `.aios-forge/squads/<slug>/agents/orquestrador.md` seguindo o template atual (seção "Step 3 — Generate the orchestrator").
...
Salve `.aios-forge/squads/<slug>/squad.md` no formato existente.

### Passo 10 — Rodar validate
Após criar tudo, execute mentalmente a task squad-validate (leia `.aios-forge/tasks/squad-validate.md`) para verificar que o pacote está consistente.
...
## Saída
- Pacote completo em `.aios-forge/squads/<slug>/`
