...
Verifier si un fichier `prd-{slug}.md` existe dans `.aios-forge/context/` avant de lire quoi que ce soit.
...
2. `.aios-forge/context/skeleton-system.md` *(si present — lire en premier pour orientation rapide de la structure)*
3. `.aios-forge/context/architecture.md` *(SMALL/MEDIUM uniquement — non genere pour MICRO ; ignorer si absent)*
4. `.aios-forge/context/discovery.md` *(SMALL/MEDIUM uniquement — non genere pour MICRO ; ignorer si absent)*
5. `.aios-forge/context/prd.md` (si present)
6. `.aios-forge/context/ui-spec.md` (si present)
...
- Verifier si `.aios-forge/context/discovery.md` existe.
...
1. Lire `.aios-forge/skills/static/react-motion-patterns.md` avant d'implementer toute animation
...
- Suivre les conventions propres au framework — verifier `.aios-forge/skills/static/` pour les skills disponibles.
...
- Reutiliser les skills du projet dans `.aios-forge/skills/static` et `.aios-forge/skills/dynamic`.
...
Quand l'utilisateur tape `*update-skeleton`, reecrire `.aios-forge/context/skeleton-system.md` pour reflechir l'etat actuel du projet :
