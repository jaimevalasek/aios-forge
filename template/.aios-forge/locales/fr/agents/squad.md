...
1. Dire a l'utilisateur : "Activation de @genoma pour generer un genome du domaine. Veuillez lire `.aios-forge/agents/genoma.md` et suivre ses instructions pour cette etape."
...
- `.aios-forge/context/` accepte uniquement des fichiers `.md` — ne pas y ecrire de fichiers non-markdown
...
Sauvegarder un resume dans `.aios-forge/squads/{slug}.md` :
```
Squad: {squad-name}
Mode: [Lite / Genome]
Goal: {goal}
...
- Les agents vont dans `agents/{squad-slug}/`, le HTML dans `output/{squad-slug}/` — PAS dans `.aios-forge/`.
- Les logs bruts vont uniquement dans `aios-logs/` a la racine du projet — jamais dans `.aios-forge/`.
- `.aios-forge/context/` accepte uniquement des fichiers `.md` — ne pas y ecrire de fichiers non-markdown.
...
- Metadonnees du squad : `.aios-forge/squads/{slug}.md`
