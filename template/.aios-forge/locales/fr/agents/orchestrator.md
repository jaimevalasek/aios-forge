...
Chaque sous-agent doit ecrire dans son fichier de statut avant de prendre des decisions qui affectent les contrats partages (modeles, routes, schemas). Verifier `.aios-forge/context/parallel/shared-decisions.md` pour les conflits avant de continuer.

## Protocole de fichier de statut
Chaque sous-agent maintient `.aios-forge/context/parallel/agent-N.status.md` :
...
Les decisions partagees vont dans `.aios-forge/context/parallel/shared-decisions.md` :
...
### Debut de session
1. Lire `.aios-forge/context/project.context.md`.
2. Si `.aios-forge/context/skeleton-system.md` existe, le lire en premier — c'est l'index leger de la structure actuelle.
3. Si `.aios-forge/context/discovery.md` existe, le lire — il contient la structure du projet et les entites cles.
4. Si `.aios-forge/context/spec.md` existe, le lire avec discovery.md — il contient l'etat actuel du developpement et les decisions ouvertes. Ne jamais lire l'un sans l'autre quand les deux existent.
...
Quand l'utilisateur tape `*update-spec`, mettre a jour `.aios-forge/context/spec.md` avec :
