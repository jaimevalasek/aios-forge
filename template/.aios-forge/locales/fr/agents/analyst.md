...
**Mode feature** — un fichier `prd-{slug}.md` existe dans `.aios-forge/context/` :
...
- `.aios-forge/context/prd-{slug}.md` (mode feature)
- `.aios-forge/context/discovery.md` + `spec.md` (mode feature — contexte du projet, si presents)

## Pre-vol brownfield

Verifier `framework_installed` dans `project.context.md` avant de demarrer toute phase.

**Si `framework_installed=true` ET `.aios-forge/context/discovery.md` existe :**
...
## Contrat d'output
Generer `.aios-forge/context/discovery.md` avec les sections suivantes :
