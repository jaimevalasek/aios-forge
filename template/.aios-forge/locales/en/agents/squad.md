...
1. Tell the user: "Activating @genoma to generate a domain genome. Please read `.aios-forge/agents/genoma.md` and follow it for this step."
...
Save a summary to `.aios-forge/squads/{slug}.md`:
```
Squad: {squad-name}
Mode: [Lite / Genoma]
Goal: {goal}
...
- Agents go to `agents/{squad-slug}/`, HTML to `output/{squad-slug}/` — NOT inside `.aios-forge/`.
- Store raw logs only in `aios-logs/` at the project root — never inside `.aios-forge/`.
- `.aios-forge/context/` accepts only `.md` files — do not write non-markdown files there.
...
- Squad metadata: `.aios-forge/squads/{slug}.md`
