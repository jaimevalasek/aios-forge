...
1. Verificar se existe perfil enriquecido em `.aios-forge/profiler-reports/{slug}/enriched-profile.md`
...
- se `.aios-forge/profiler-reports/{slug}/enriched-profile.md` existir:
...
> [2] Salvar localmente (.aios-forge/genomas/[slug].md + .aios-forge/genomas/[slug].meta.json)
...
Salvar:
- `.aios-forge/genomas/[slug-domínio].md`
- `.aios-forge/genomas/[slug-domínio].meta.json`
...
- Se `MAKOPY_KEY` não estiver configurada:
  ...
  > Para publicar: `aios-forge config set MAKOPY_KEY=mk_live_xxx`
...
- Atualizar `.aios-forge/squads/{slug}.md` com:
...
1. Ler `.aios-forge/tasks/squad-validate.md` e executar mentalmente
...
- Se aplicar o genoma a um squad/agente, persista esse vínculo em `.aios-forge/squads/{slug}.md`
- Não modifique agentes oficiais de `.aios-forge/agents/` com genomas customizados do usuário
- `.aios-forge/context/` aceita somente `.md`. Não escreva arquivos não-markdown lá.

## Contrato de output

- Arquivo de genoma (se salvo): `.aios-forge/genomas/[slug].md`
- Arquivo de metadata do genoma (se salvo): `.aios-forge/genomas/[slug].meta.json`
- Valor de retorno para @squad: conteúdo completo do genoma
- Vínculo persistente, quando aplicado: `.aios-forge/squads/{slug}.md`
