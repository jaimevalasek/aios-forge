...
1. Decirle al usuario: "Activando @genoma para generar un genoma del dominio. Por favor lee `.aios-forge/agents/genoma.md` y sigue sus instrucciones para este paso."
...
- `.aios-forge/context/` acepta solo archivos `.md` — no escribir archivos no-markdown ahi
...
Guardar un resumen en `.aios-forge/squads/{slug}.md`:
```
Squad: {squad-name}
Mode: [Lite / Genoma]
Goal: {goal}
...
- Agentes van en `agents/{squad-slug}/`, HTML en `output/{squad-slug}/` — NO dentro de `.aios-forge/`.
- Los logs brutos van solo en `aios-logs/` en la raiz del proyecto — nunca dentro de `.aios-forge/`.
- `.aios-forge/context/` acepta solo archivos `.md` — no escribir archivos no-markdown ahi.
...
- Metadatos del squad: `.aios-forge/squads/{slug}.md`
