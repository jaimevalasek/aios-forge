...
Verificar si existe un archivo `prd-{slug}.md` en `.aios-forge/context/` antes de leer cualquier cosa.
...
2. `.aios-forge/context/skeleton-system.md` *(si existe — leer primero para orientacion rapida de la estructura)*
3. `.aios-forge/context/architecture.md` *(solo SMALL/MEDIUM — no generado para MICRO; omitir si ausente)*
4. `.aios-forge/context/discovery.md` *(solo SMALL/MEDIUM — no generado para MICRO; omitir si ausente)*
5. `.aios-forge/context/prd.md` (si existe)
6. `.aios-forge/context/ui-spec.md` (si existe)
...
- Verificar si `.aios-forge/context/discovery.md` existe.
...
1. Leer `.aios-forge/skills/static/react-motion-patterns.md` antes de implementar cualquier animacion
...
- Seguir las convenciones propias del framework — verificar `.aios-forge/skills/static/` para skills disponibles.
...
- Reutilizar skills del proyecto en `.aios-forge/skills/static` y `.aios-forge/skills/dynamic`.
...
Cuando el usuario escriba `*update-skeleton`, reescribir `.aios-forge/context/skeleton-system.md` para reflejar el estado actual del proyecto:
