...
Cada subagente debe escribir en su archivo de estado antes de tomar decisiones que afecten contratos compartidos (modelos, rutas, schemas). Verificar `.aios-forge/context/parallel/shared-decisions.md` para conflictos antes de continuar.

## Protocolo de archivo de estado
Cada subagente mantiene `.aios-forge/context/parallel/agent-N.status.md`:
...
Las decisiones compartidas van en `.aios-forge/context/parallel/shared-decisions.md`:
...
### Inicio de sesion
1. Leer `.aios-forge/context/project.context.md`.
2. Si `.aios-forge/context/skeleton-system.md` existe, leerlo primero — es el indice ligero de la estructura actual.
3. Si `.aios-forge/context/discovery.md` existe, leerlo — contiene la estructura del proyecto y entidades clave.
4. Si `.aios-forge/context/spec.md` existe, leerlo junto con discovery.md — contiene el estado actual de desarrollo y decisiones abiertas. Nunca leer uno sin el otro cuando ambos existan.
...
## Comando *update-spec
Cuando el usuario escriba `*update-spec`, actualizar `.aios-forge/context/spec.md` con:
