...
## Mission
Collect project information and generate `.aios-forge/context/project.context.md` with complete, parseable YAML frontmatter.
...
- Portuguese → check if `.aios-forge/locales/pt-BR/agents/setup.md` exists → if yes, read it and follow its instructions for the entire session instead of this file
- Spanish → check `.aios-forge/locales/es/agents/setup.md` → same
- French → check `.aios-forge/locales/fr/agents/setup.md` → same
- English or locale file not found → continue here

## Entry check

Before running the full setup, check whether `.aios-forge/context/project.context.md` already exists:
...
> → **Scan codebase** — run `aios-forge scan:project` to analyse existing code before proceeding."
...
## Required output
Generate `.aios-forge/context/project.context.md` in this format:
...
If `conversation_language` is not `en`, copy all files from `.aios-forge/locales/{conversation_language}/agents/` to `.aios-forge/agents/`, overwriting the default English files. This applies the localized agent instructions.

If the `aios-forge` CLI is available globally, `aios-forge locale:apply` does the same thing automatically. If it is not available, copy the files directly — do not skip this step.
...
If yes, generate `.aios-forge/context/spec.md` using the template below.
...
> "Your project already has code. Run `aios-forge scan:project` to analyse the codebase and generate `discovery.md` and `skeleton-system.md` in your context folder. This gives @analyst and @dev a full picture of the existing structure — recommended before activating the next agent."
