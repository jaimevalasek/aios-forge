...
- Portuguese -> check if `.aios-forge/locales/pt-BR/agents/genoma.md` exists -> if yes, read it and follow it
- Spanish -> check `.aios-forge/locales/es/agents/genoma.md` -> same
- French -> check `.aios-forge/locales/fr/agents/genoma.md` -> same
...
> [2] Save locally (.aios-forge/genomas/[slug].md + .aios-forge/genomas/[slug].meta.json)
...
Save:
- `.aios-forge/genomas/[domain-slug].md`
- `.aios-forge/genomas/[domain-slug].meta.json`
...
- If `MAKOPY_KEY` is not configured:
  ...
  > To publish: `aios-forge config set MAKOPY_KEY=mk_live_xxx`
...
- Update `.aios-forge/squads/{slug}.md` with:
...
1. Read `.aios-forge/tasks/squad-validate.md` and execute mentally
...
- If applying the genome to a squad/agent, persist that binding in `.aios-forge/squads/{slug}.md`
- Do not modify official `.aios-forge/agents/` files with user custom genomes
- `.aios-forge/context/` accepts only `.md` files. Do not write non-markdown files there.

## Output contract

- Genome file (if saved): `.aios-forge/genomas/[slug].md`
- Genome metadata file (if saved): `.aios-forge/genomas/[slug].meta.json`
- Return value to @squad: full genome content
- Persistent binding when applied: `.aios-forge/squads/{slug}.md`
