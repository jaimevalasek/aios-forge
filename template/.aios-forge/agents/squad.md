...
- Portuguese → check if `.aios-forge/locales/pt-BR/agents/squad.md` exists → if yes, read it and follow its instructions for the entire session instead of this file
- Spanish → check `.aios-forge/locales/es/agents/squad.md` → same
- French → check `.aios-forge/locales/fr/agents/squad.md` → same
...
A squad is a **team of real, invocable agent files** created at `.aios-forge/squads/{squad-slug}/agents/`.
...
AIOS Forge supports multiple parallel squads in the same project.
...
- `@squad design <slug>` → read and execute `.aios-forge/tasks/squad-design.md`
- `@squad create <slug>` → read and execute `.aios-forge/tasks/squad-create.md`
- `@squad validate <slug>` → read and execute `.aios-forge/tasks/squad-validate.md`
- `@squad analyze <slug>` → read and execute `.aios-forge/tasks/squad-analyze.md` (Fase 3)
- `@squad extend <slug>` → read and execute `.aios-forge/tasks/squad-extend.md` (Fase 3)
- `@squad repair <slug>` → read and execute `.aios-forge/tasks/squad-repair.md` (Fase 4)
- `@squad export <slug>` → read and execute `.aios-forge/tasks/squad-export.md` (Fase 3)
- `@squad pipeline <sub> [args]` → read and execute `.aios-forge/tasks/squad-pipeline.md`
...
- update `.aios-forge/squads/{slug}/squad.md`
- record whether the genome applies to the whole squad or to specific agents only
- rewrite the affected agent files in `.aios-forge/squads/{squad-slug}/agents/` so they include the newly active genome
...
1. Look for the artisan PRD at `.aios-forge/squads/.artisan/<id>.md` (filesystem fallback)
...
- a package root at `.aios-forge/squads/{squad-slug}/`
- a short manifesto at `.aios-forge/squads/{squad-slug}/agents/agents.md`
- a structured manifest at `.aios-forge/squads/{squad-slug}/squad.manifest.json`
- skills at `.aios-forge/squads/{squad-slug}/skills/`
- templates at `.aios-forge/squads/{squad-slug}/templates/`
- a local `design-doc` at `.aios-forge/squads/{squad-slug}/docs/design-doc.md`
- a local `readiness` file at `.aios-forge/squads/{squad-slug}/docs/readiness.md`
- permanent executors in `.aios-forge/squads/{squad-slug}/agents/`
- metadata at `.aios-forge/squads/{slug}/squad.md`
...
- also check whether the squad already has installed skills in `.aios-forge/squads/{squad-slug}/skills/` before creating new or duplicate skills
...
> **Project rules**: Before starting, check `.aios-forge/rules/` in the project root.
...
> Also check `.aios-forge/docs/` for reference docs relevant to the current task.
```
...
- `.aios-forge/squads/{squad-slug}/skills/{domain}/{skill-slug}.md`
...
Create `.aios-forge/squads/{squad-slug}/agents/agents.md`:
...
Also create `.aios-forge/squads/{squad-slug}/squad.manifest.json` with this minimum schema:
...
If domain-specific fields are needed, define them inside the squad blueprint, never as a fixed global AIOS Forge rule.

### Step 2 — Generate each specialist agent

For each role, create `.aios-forge/squads/{squad-slug}/agents/{role-slug}.md`:
...
Create `.aios-forge/squads/{squad-slug}/agents/orquestrador.md`:
...
- `.aios-forge/context/` accepts only `.md` files — do not write non-markdown files there
...
Save a summary to `.aios-forge/squads/{slug}/squad.md`:
```
Squad: {squad-name}
Mode: Squad
Goal: {goal}
...
Agents created in `.aios-forge/squads/{squad-slug}/agents/`:
- @{role1} — [one-line description]
- @{role2} — [one-line description]
- @{role3} — [one-line description]
...
Squad manifests created at `.aios-forge/squads/{squad-slug}/agents/agents.md` and `.aios-forge/squads/{squad-slug}/squad.manifest.json`.
```
...
- Do NOT use `squads/active/squad.md` — agents go to `.aios-forge/squads/{squad-slug}/agents/`, HTML to `output/{squad-slug}/`.
- Store raw logs only in `aios-logs/{squad-slug}/` at the project root — never inside `.aios-forge/`.
...
- Agent files: `.aios-forge/squads/{squad-slug}/agents/` (editable by user, invocable via `@`)
- Squad text manifesto: `.aios-forge/squads/{squad-slug}/agents/agents.md`
- Squad JSON manifest: `.aios-forge/squads/{squad-slug}/squad.manifest.json`
- Squad metadata: `.aios-forge/squads/{slug}/squad.md`
- Session HTMLs: `output/{squad-slug}/{session-id}.html`
- Latest HTML: `output/{squad-slug}/latest.html`
- Draft `.md` files: `output/{squad-slug}/`
- Genome bindings: `.aios-forge/squads/{slug}/squad.md`
- Logs: `aios-logs/{squad-slug}/`
- Media: `media/{squad-slug}/`
