...
1. **`.aios-forge/rules/`** — If this directory exists, list its `.md` files. For each:
   - Read YAML frontmatter. If `agents:` is absent → load (universal rule).
   - If `agents:` includes `dev` → load. Otherwise skip.
   - Loaded rules **override** the default conventions in this file.
2. **`.aios-forge/docs/`** — If this directory exists, load doc files whose `description` frontmatter is relevant to the current task, or when explicitly mentioned by the user.

## Feature mode detection

Check whether a `prd-{slug}.md` file exists in `.aios-forge/context/` before reading anything else.
...
2. `.aios-forge/context/skeleton-system.md` *(if present — read first for quick structural orientation)*
3. `.aios-forge/context/design-doc.md` *(if present — treat as the current scope decision document)*
4. `.aios-forge/context/readiness.md` *(if present — verify the scope is ready for implementation)*
5. `.aios-forge/context/architecture.md` *(SMALL/MEDIUM only — not generated for MICRO; skip if absent)*
6. `.aios-forge/context/discovery.md` *(SMALL/MEDIUM only — not generated for MICRO; skip if absent)*
7. `.aios-forge/context/prd.md` (if present)
8. `.aios-forge/context/ui-spec.md` (if present)
...
- Check whether `.aios-forge/context/discovery.md` exists.
...
1. Read `.aios-forge/skills/static/react-motion-patterns.md` before implementing any animation
...
- Follow the framework's own conventions — check `.aios-forge/skills/static/` for available skill files.
...
- Reuse project skills in `.aios-forge/skills/static` and `.aios-forge/skills/dynamic`.
- Also reuse squad-installed skills in `.aios-forge/squads/{squad-slug}/skills/` when the task belongs to a squad package.
...
When the user types `*update-skeleton`, rewrite `.aios-forge/context/skeleton-system.md` to reflect the current state of the project:
...
> **`.aios-forge/context/` rule:** this folder accepts only `.md` files. Never write `.html`, `.css`, `.js`, or any other non-markdown file inside `.aios-forge/`.
