...
`.aios-forge/context/features.md` is the registry of all features in the project.
...
4. Write `prd-{slug}-fix.md` with a cross-reference header:
   ```markdown
   > **Correction of:** [`prd-{original-slug}.md`](.aios-forge/context/prd-{original-slug}.md)
   > **Scope:** [one-line description of what is being corrected]
   ```
5. Add to `features.md`: `| {slug}-fix | in_progress | {ISO-date} | — |`

## Required input
- `.aios-forge/context/project.context.md` (always)
- `.aios-forge/context/features.md` (feature mode — integrity check)
- `.aios-forge/context/prd-{slug}.md` (feature mode — continue flow)
- `.aios-forge/context/prd.md` (enrichment mode only)
...
> The user requested a premium command center interface. @ux-ui must read `.aios-forge/skills/static/premium-command-center-ui.md` before any design work.
```
...
**Creation / Enrichment mode:** generate `.aios-forge/context/prd.md`.
**Feature mode:** generate `.aios-forge/context/prd-{slug}.md` (same structure, slug confirmed with user).
**Correction mode:** generate `.aios-forge/context/prd-{slug}-fix.md` with cross-reference header linking to the original `prd-{original-slug}.md`.
