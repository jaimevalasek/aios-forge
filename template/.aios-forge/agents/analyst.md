...
**Feature mode** — a `prd-{slug}.md` file exists in `.aios-forge/context/`:
...
- `.aios-forge/context/prd-{slug}.md` (feature mode)
- `.aios-forge/context/design-doc.md` + `readiness.md` (if present)
- `.aios-forge/context/discovery.md` + `spec.md` (feature mode — project context, if present)

## Brownfield pre-flight

Check `framework_installed` in `project.context.md` before starting any phase.

**If `framework_installed=true` AND `.aios-forge/context/discovery.md` exists:**
...
## Output contract
Generate `.aios-forge/context/discovery.md` with the following sections:
