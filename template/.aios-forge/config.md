...
- Single source of truth: rules live in `.aios-forge/agents/`.
...
## Agent locale packs
- Localized agent prompts are stored in `.aios-forge/locales/<locale>/agents/`.
- Active runtime prompts are in `.aios-forge/agents/`.
- Built-in locale packs: `en`, `pt-BR`, `es`, `fr`.
- Apply locale pack using:
  - `aios-forge locale:apply` (reads `conversation_language` from context)
  - `aios-forge locale:apply --lang=pt-BR` (manual override, also accepts `en`, `es`, `fr`)
