...
def build_prompt(tree_lines, key_contents, project_context, spec_content):
    ...
    parts.append("""
## Task
Generate `.aios-lite/context/discovery.md` with exactly these sections:
...
---
_Generated by aios-lite-scan.py — """ + datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ") + "_\n"
    )
...
def main():
    ...
    print(f"\n✓ {OUTPUT_FILE} written ({len(result)} chars)")
    print("\n  Next steps:")
    print("  1. Open your AI coding session (Claude Code, Codex, etc.)")
    print("  2. Run @analyst — it will read discovery.md automatically")
    print("  3. Run @dev    — it will read discovery.md + spec.md together")
