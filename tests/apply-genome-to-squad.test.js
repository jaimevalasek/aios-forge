...
async function makeTempDir() {
  return fs.mkdtemp(path.join(os.tmpdir(), 'aios-lite-apply-genome-'));
}

test('applyGenomeToExistingSquad persists bindings to blueprint, manifest and readiness', async () => {
  const projectRoot = await makeTempDir();
  const slug = 'content-lab';
  const squadDir = path.join(projectRoot, '.aios-lite', 'squads', slug);
  const designsDir = path.join(projectRoot, '.aios-lite', 'squads', '.designs');
        ...
        executors: [
          {
            slug: 'writer',
            role: 'Writes content',
            file: `.aios-lite/squads/${slug}/agents/writer.md`,
            genomes: ['legacy-voice']
          },
          {
            slug: 'reviewer',
            role: 'Reviews content',
            file: `.aios-lite/squads/${slug}/agents/reviewer.md`
          }
        ]
      },
      null,
